...
public class ScheduleController {
    ...
    public ResponseEntity<Page<ScheduleEntity>> findAll(
            @PathVariable("pageno") int pageNo,
            @PathVariable("pagesize") int pageSize,
            @PathVariable("sortby") String sortColumn,
            @PathVariable("sortdirection") String sortdirection,
            @RequestBody List<FilterClause> filterClauses) {
        return new ResponseEntity<Page<ScheduleEntity>>(scheduleService.findAll(pageNo, pageSize, sortColumn, sortdirection == null ? "asc" : sortdirection, filterClauses), HttpStatus.OK);
    }


    @GetMapping("/findbyid/{id}")
    public ResponseEntity<ScheduleEntity> findById(@PathVariable("id") Integer id) {
        return new ResponseEntity<ScheduleEntity>(scheduleService.findById(id), HttpStatus.OK);
    }

    @PostMapping("/save")
    public ResponseEntity<AccountResponse> save(@RequestBody ScheduleDto scheduleDto) {
        AccountResponse response = new AccountResponse();
        try {
            ScheduleEntity entity = scheduleService.save(scheduleDto);
    ...
    @PutMapping("/modify")
    public ResponseEntity<AccountResponse> update(@RequestBody ScheduleDto scheduleDto) {
        AccountResponse response = new AccountResponse();
        try {
            ScheduleEntity entity = scheduleService.save(scheduleDto);
            response.setId(entity.getId());
    ...
    public ResponseEntity<AccountResponse> remove(@RequestBody List<Integer> ids) {
        AccountResponse response = new AccountResponse();
        try {
            List<ScheduleEntity> entities = scheduleService.findAllEntitiesByIds(ids);
            entities.stream().forEach(entity -> {
                entity.setModifiedBy(UserContext.getLoggedInUser());
                entity.setIsDeleted(1);
            });
            scheduleService.removeAll(entities);
            response.setId(null);
            response.setStatus(100);
            response.setLangStatus("modify_100");
            response.setMessage(MessageConstants.RECORD_DELETED);
    ...
    public ResponseEntity<AccountResponse> unlock(@RequestBody List<Integer> ids) {
        ...
        try {
            ...
            scheduleService.unlock(entities);
            response.setId(null);
            response.setStatus(100);
            response.setLangStatus("lock_100");
