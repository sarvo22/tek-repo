...
@Slf4j
@RestController
@RequestMapping("/account/subgroup")
public class SubGroupController {
    ...
    public ResponseEntity<Page<SubGroupEntity>> findAll(
            @PathVariable("pageno") int pageNo,
            @PathVariable("pagesize") int pageSize,
            @PathVariable("sortby") String sortColumn,
            @PathVariable("sortdirection") String sortdirection,
            @RequestBody List<FilterClause> filterClauses) {
        return new ResponseEntity<Page<SubGroupEntity>>(subgroupService.findAll(pageNo, pageSize, sortColumn, sortdirection == null ? "asc" : sortdirection, filterClauses), HttpStatus.OK);
    }

    @GetMapping("/findbyid/{id}")
    public ResponseEntity<SubGroupEntity> findById(@PathVariable("id") Integer id) {
        return new ResponseEntity<SubGroupEntity>(subgroupService.findById(id), HttpStatus.OK);
    }

    @PostMapping("/save")
    public ResponseEntity<AccountResponse> save(@RequestBody SubGroupDto subgroupDto) {
        AccountResponse response = new AccountResponse();
        try {
            SubGroupEntity entity = subgroupService.save(subgroupDto);
    ...
    @PutMapping("/modify")
    public ResponseEntity<AccountResponse> update(@RequestBody SubGroupDto subgroupDto) {
        AccountResponse response = new AccountResponse();
        try {
            SubGroupEntity entity = subgroupService.save(subgroupDto);
            response.setId(entity.getId());
    ...
    public ResponseEntity<AccountResponse> remove(@PathVariable("id") Integer id,
        ...
        AccountResponse response = new AccountResponse();
        try {
            SubGroupEntity entity = subgroupService.findById(id);
            entity.setModifiedBy(UserContext.getLoggedInUser());
            entity.setIsDeleted(1);
            subgroupService.remove(entity);
            response.setId(entity.getId());
            response.setStatus(100);
            response.setLangStatus("modify_100");
    ...
    public ResponseEntity<AccountResponse> remove(@RequestBody List<Integer> ids) {
        AccountResponse response = new AccountResponse();
        try {
            List<SubGroupEntity> entities = subgroupService.findAllEntitiesByIds(ids);
            entities.stream().forEach(entity -> {
                entity.setModifiedBy(UserContext.getLoggedInUser());
                entity.setIsDeleted(1);
            });
            subgroupService.removeAll(entities);
            response.setId(null);
            response.setStatus(100);
            response.setLangStatus("modify_100");
            response.setMessage(MessageConstants.RECORD_DELETED);
    ...
    public ResponseEntity<AccountResponse> unlock(@RequestBody List<Integer> ids) {
        ...
        try {
            ...
            subgroupService.unlock(entities);
            response.setId(null);
            response.setStatus(100);
            response.setLangStatus("lock_100");
