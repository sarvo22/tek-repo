...
@Slf4j
@RestController
@RequestMapping("/account/ifrsgroup")
public class IfrsGroupController {

    @Autowired
    IfrsGroupService ifrsGroupService;

    @PostMapping("/search/{pageno}/{pagesize}/{sortby}/{sortdirection}")
    public ResponseEntity<Page<IfrsGroupEntity>> findAll(
            @PathVariable("pageno") int pageNo,
            @PathVariable("pagesize") int pageSize,
            @PathVariable("sortby") String sortColumn,
            @PathVariable("sortdirection") String sortdirection,
            @RequestBody List<FilterClause> filterClauses) {
        return new ResponseEntity<Page<IfrsGroupEntity>>(ifrsGroupService.findAll(pageNo, pageSize, sortColumn, sortdirection == null ? "asc" : sortdirection, filterClauses), HttpStatus.OK);
    }

    @GetMapping("/findbyid/{id}")
    public ResponseEntity<IfrsGroupEntity> findById(@PathVariable("id") Integer id) {
        return new ResponseEntity<IfrsGroupEntity>(ifrsGroupService.findById(id), HttpStatus.OK);
    }

    @PostMapping("/save")
    public ResponseEntity<AccountResponse> save(@RequestBody IfrsGroupDto ifrsGroupDto) {
        AccountResponse response = new AccountResponse();
        try {
            IfrsGroupEntity entity = ifrsGroupService.save(ifrsGroupDto);
    ...
    @PutMapping("/modify")
    public ResponseEntity<AccountResponse> update(@RequestBody IfrsGroupDto ifrsGroupDto) {
        AccountResponse response = new AccountResponse();
        try {
            IfrsGroupEntity entity = ifrsGroupService.save(ifrsGroupDto);
            response.setId(entity.getId());
    ...
    public ResponseEntity<AccountResponse> remove(@PathVariable("id") Integer id,
        ...
        AccountResponse response = new AccountResponse();
        try {
            IfrsGroupEntity entity = ifrsGroupService.findById(id);
            entity.setModifiedBy(UserContext.getLoggedInUser());
            entity.setIsDeleted(1);
            ifrsGroupService.remove(entity);
            response.setId(entity.getId());
            response.setStatus(100);
            response.setLangStatus("modify_100");
    ...
    public ResponseEntity<AccountResponse> remove(@RequestBody List<Integer> ids) {
        AccountResponse response = new AccountResponse();
        try {
            List<IfrsGroupEntity> entities = ifrsGroupService.findAllEntitiesByIds(ids);
            entities.stream().forEach(entity -> {
                entity.setModifiedBy(UserContext.getLoggedInUser());
                entity.setIsDeleted(1);
            });
            ifrsGroupService.removeAll(entities);
            response.setId(null);
            response.setStatus(100);
            response.setLangStatus("modify_100");
            response.setMessage(MessageConstants.RECORD_DELETED);
    ...
    public ResponseEntity<AccountResponse> lock(@RequestBody List<Integer> ids) {
        ...
        try {
            ...
            ifrsGroupService.lock(entities);
            response.setId(null);
            response.setStatus(100);
    ...
    public ResponseEntity<AccountResponse> unlock(@RequestBody List<Integer> ids) {
        ...
        try {
            ...
            ifrsGroupService.unlock(entities);
            response.setId(null);
            response.setStatus(100);
            response.setLangStatus("lock_100");
