...
@Data
@Entity
@Table(name = "tbl_sales_invoice_main")
public class SalesInvoiceMainEntity {

    @Id
    @GeneratedValue(strategy = GenerationType.SEQUENCE, generator = "salesinvoicemain_generator")
    @SequenceGenerator(name = "salesinvoicemain_generator", sequenceName = "tbl_sales_invoice_main_seq", allocationSize = 1)
...
//    @OneToMany(targetEntity = SalesInvoiceDetailEntity.class, fetch = FetchType.EAGER)
//    @JoinColumn(name = "inv_id", referencedColumnName = "inv_id", insertable = false, updatable = false)
//    private List<SalesInvoiceDetailEntity> detailEntityList;
}
