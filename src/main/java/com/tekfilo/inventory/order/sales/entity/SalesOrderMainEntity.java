...
@Setter
@Entity
@Table(name = "tbl_sales_order_main")
public class SalesOrderMainEntity {

    @Id
    @GeneratedValue(strategy = GenerationType.SEQUENCE, generator = "salesorder_generator")
    @SequenceGenerator(name = "salesorder_generator", sequenceName = "tbl_sales_order_main_seq", allocationSize = 1)
    @Column(name = "sales_order_id", updatable = false, insertable = true, nullable = false)
    private Integer id;

    @Column(name = "sales_order_type", insertable = true, updatable = false)
    private String salesOrderType;

    @Column(name = "sales_order_no", insertable = true, updatable = false)
