...
public interface PurchaseOrderDetailRepository extends JpaRepository<PurchaseOrderDetailEntity, Integer> {
    ...
    @Modifying
    @Transactional
    @Query("update PurchaseOrderDetailEntity set isDeleted = 1, modifiedBy=:modifiedBy, modifiedDt=:modifiedDt where isDeleted = 0 and purchaseOrderId in (:ids)")
    void deleteDetailByPurchaseOrderId(List<Integer> ids, Integer modifiedBy, Timestamp modifiedDt) throws SQLException;
}
